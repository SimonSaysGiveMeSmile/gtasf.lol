...
function PedestrianNPC({ npc }: NPCCProps) {
  ...
  const pos = useRef<[number, number, number]>(npc.position)
  const vel = useRef<[number, number, number]>([0, 0, 0])
  const stateTimer = useRef(0)
  const targetPos = useRef<[number, number, number]>([
    npc.position[0] + (seededRandom(parseInt(npc.id)) - 0.5) * 30,
    0,
    npc.position[2] + (seededRandom(parseInt(npc.id) * 2) - 0.5) * 30,
  ])
...
function TrafficNPC({ npc }: NPCCProps) {
  ...
  useFrame((_, delta) => {
    ...
    // Simple driving AI - go in circles/rectangles
    const speed = 15
    const forward: [number, number, number] = [
      -Math.sin(rotation.current) * speed,
      0,
      -Math.cos(rotation.current) * speed,
    ]

    physApi.velocity.set(forward[0], vel.current[1], forward[2])
