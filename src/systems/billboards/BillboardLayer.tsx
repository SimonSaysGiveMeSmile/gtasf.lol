...
export default function BillboardLayer() {
  const billboards = useMemo(() => {
    ...
    for (let pi = 0; pi < paths.length; pi++) {
      const path = paths[pi]
      // Only main/busier roads get billboards (every other road)
      if (pi % 2 !== 0) continue
      for (let si = 5; si < path.length - 5; si += 4) {
        const side: 'left' | 'right' = (si / 4) % 2 === 0 ? 'left' : 'right'
        const spot = getBillboardSpot(pi, si, side)
        if (!spot) continue
        if (!isClearOfBuildings(spot.x, spot.z, 5)) continue

        const ad = BILLBOARD_ADS[idx % BILLBOARD_ADS.length]
        const height = 6 + seededRandom(idx * 37) * 5 // varied pole heights
    ...
    for (let pi = 0; pi < paths.length && spots.length < 30; pi++) {
      const path = paths[pi]
      // Intersections are at segment indices 0 and mid points
      for (const si of [0, Math.floor(path.length / 2)]) {
        ...
        if (!spot) continue
        if (!isClearOfBuildings(spot.x, spot.z, 5)) continue
        // Skip if too close to existing billboard
        let tooClose = false
